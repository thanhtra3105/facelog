"""
verification_yunet.py — Xác thực khuôn mặt realtime bằng YuNet + kiểm tra khoảng cách/chất lượng.

Chạy ví dụ:
  python verification_yunet.py --model "E:/THANHTRA/KI_8/AI/facelog/models/arcface_vggface2.pth" --yunet face_detection_yunet_2023mar.onnx
"""
...
# ── Cấu hình mặc định ──────────────────────────────────────────────────────
THRESHOLD_OPEN = 0.80       # Nên calibration theo dữ liệu thật, đừng hạ bừa cho khóa cửa
THRESHOLD_UNSURE = 0.70
SMOOTH_WINDOW = 8
REQUIRED_SAME_NAME = 6      # phải cùng tên ít nhất 6/8 frame mới mở
LOCK_COOLDOWN = 3.0
MIN_FACE_RATIO_VERIFY = 0.075  # mặt quá nhỏ thì yêu cầu tiến lại gần
MAX_FACE_RATIO_VERIFY = 0.45   # mặt quá sát thì yêu cầu lùi ra
...
def identify(embedding, db, unsure_threshold: float):
    """So sánh embedding với toàn bộ DB. Trả về (name, best_score)."""
    if not db:
        return "UNKNOWN", 0.0
    ...
    for name, data in db.items():
        ...
        if not refs:
            continue
        scores = [cosine_similarity(embedding, ref) for ref in refs]
        score = max(scores)
        if score > best_score:
            best_score = score
            best_name = name
...
def draw_verification_ui(frame, name, score, status, door_open, fps, msg=""):
    ...
    if door_open:
        ...
        cv2.putText(frame, "WELCOME!", (w // 2 - 100, h // 2), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 120), 3)
    ...
    while True:
        ...
        if face is not None:
            ...
            if r < MIN_FACE_RATIO_VERIFY:
                ...
                name, raw_score = identify(emb, db, threshold_unsure)
                ...
                if (
                    ...
                    if not door_open and (now - last_open_time >= LOCK_COOLDOWN):
                        ...
                        door_open_time = now
                        last_open_time = now
                        print(f"  🔓 MỞ CỬA — {most_common} (score={smooth_score:.3f}, same={same_count}/{SMOOTH_WINDOW})")
                ...
                name = most_common
                score = smooth_score
                msg = f"ratio={r:.2f} blur={bscore:.0f} same={same_count}/{SMOOTH_WINDOW}"
        else:
            score_history.clear()
            name_history.clear()
            status = "WAITING"
