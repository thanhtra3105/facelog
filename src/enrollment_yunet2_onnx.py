"""
enrollment_yunet.py — Đăng ký khuôn mặt bằng YuNet + nhiều khoảng cách/góc nhìn (ONNX VERSION)

Chuẩn bị:
  1) Cài OpenCV và ONNX Runtime: pip install -U opencv-python onnxruntime numpy
  2) Tải model YuNet: face_detection_yunet_2023mar.onnx
  3) Model ArcFace định dạng ONNX: arcface_vggface2.onnx

Chạy ví dụ:
  python enrollment_yunet.py --name "Le Thanh Tra" --model "E:/HK8/TTNT/QuangDaAI/facelog/models/arcface_vggface2.onnx" --yunet "E:/HK8/TTNT/QuangDaAI/facelog/models/face_detection_yunet_2023mar.onnx"
"""
...
# ── ONNX Preprocessing & Extraction ─────────────────────────
def preprocess_face(face_bgr):
    face_rgb = cv2.cvtColor(face_bgr, cv2.COLOR_BGR2RGB)
    # Chuẩn hóa (img - 127.5) / 127.5 thay thế cho torchvision Normalize
...
def enroll(
    ...
    print(f"\n[THANH CONG] Đã đăng ký thành công: {name}")
...
if __name__ == "__main__":
    ...
    # Đã cập nhật default path sang file .onnx
