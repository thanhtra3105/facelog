"""
enrollment.py — Thu thập khuôn mặt từ 4 hướng + trích xuất embedding (ONNX VERSION)
...
# ── Enrollment App ─────────────────────────────────────────────────────────
DIRECTIONS = [
    ("THANG",   "Nhin thang vao camera",        3.0),
    ("TRAI",    "Quay mat sang TRAI nhe (~30*)",  3.0),
    ("PHAI",    "Quay mat sang PHAI nhe (~30*)",  3.0),
    ("LEN",     "Ngang dau len nhe",              3.0),
    ("XUONG",   "Cui dau xuong nhe",              3.0),
]
...
def preprocess_face(face_bgr):
    """Thay thế torchvision.transforms bằng numpy"""
    face_rgb = cv2.cvtColor(face_bgr, cv2.COLOR_BGR2RGB)
    
    # Ép kiểu float32 và chuẩn hóa tương đương transforms.Normalize([0.5]*3, [0.5]*3)
    # Công thức: (pixel/255.0 - 0.5) / 0.5 <=> (pixel - 127.5) / 127.5
...
def extract_embedding(session, face_bgr):
    # Tiền xử lý
    ...
    # Normalize vector với L2 norm (tương đương nn.functional.normalize(p=2))
...
def enroll(name: str, model_path: str):
    print(f"[INFO] Khởi tạo ONNX Runtime...")
    
    # Load ONNX model
    if os.path.exists(model_path):
        session = ort.InferenceSession(model_path)
        print(f"[INFO] Đã load mô hình: {model_path}")
    else:
        ...
        while True:
            ...
            if collecting and start_time:
                ...
                if elapsed >= duration or collected_frames >= TARGET_FRAMES:
                    print(f"     v Thu thập {len(embeddings_this_dir)} embeddings")
                    break
    ...
    save_db(db)
    print(f"\n[THANH CONG] Đã đăng ký cho: {name}")
