#!/usr/bin/env python3
"""
register_upload_admin.py — Dang ky khuon mat bang anh upload, co dang nhap admin.

Chay:
    export ADMIN_USER=admin
    export ADMIN_PASSWORD=doi_mat_khau_manh
    python register_upload_admin.py
...
Can cai:
    pip install flask numpy pillow ai-edge-litert
    sudo apt install -y python3-opencv

Neu dang dung Python 3.13 + venv --system-site-packages:
...
# Tai khoan admin. Nen set bang bien moi truong truoc khi chay:
#   export ADMIN_USER=admin
#   export ADMIN_PASSWORD=mat_khau_manh
...
<style>
*{box-sizing:border-box} body{margin:0;background:#0f0f0f;color:#e8e8e8;font-family:Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:18px}
.card{width:100%;max-width:380px;background:#181818;border:1px solid #2a2a2a;border-radius:14px;padding:20px}
h1{font-size:1.2rem;margin:0 0 6px}.muted{color:#999;font-size:.9rem;margin-bottom:14px}
label{display:block;margin:10px 0 6px;color:#ccc;font-size:.9rem}
input{width:100%;padding:11px;border-radius:8px;border:1px solid #333;background:#111;color:#eee}
button{width:100%;margin-top:14px;border:0;border-radius:8px;padding:11px 14px;font-weight:700;cursor:pointer;background:#2563eb;color:white}
.err{background:#3a1515;color:#fca5a5;border:1px solid #7f1d1d;border-radius:8px;padding:8px;margin:10px 0;font-size:.9rem}
.small{margin-top:12px;font-size:.8rem;color:#777;line-height:1.35}
</style>
</head>
<body>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Dang ky khuon mat bang anh upload</title>
<style>
*{box-sizing:border-box} body{margin:0;background:#0f0f0f;color:#e8e8e8;font-family:Arial,sans-serif;padding:18px}
.container{max-width:860px;margin:0 auto;display:grid;gap:14px}
.card{background:#181818;border:1px solid #2a2a2a;border-radius:12px;padding:16px}
h1{font-size:1.15rem;margin:0 0 4px;color:#fff} .muted{color:#999;font-size:.9rem}
.row{display:grid;grid-template-columns:1fr 1fr;gap:14px}@media(max-width:760px){.row{grid-template-columns:1fr}}
label{display:block;margin:10px 0 6px;color:#ccc;font-size:.9rem}
input[type=text],input[type=number]{width:100%;padding:10px;border-radius:8px;border:1px solid #333;background:#111;color:#eee}
input[type=file]{width:100%;padding:10px;border-radius:8px;border:1px solid #333;background:#111;color:#eee}
button{border:0;border-radius:8px;padding:10px 14px;font-weight:700;cursor:pointer;background:#2563eb;color:white}
button:disabled{background:#333;color:#777;cursor:not-allowed}.danger{background:#7f1d1d}.ok{color:#22c55e}.warn{color:#f59e0b}.err{color:#f87171}
#preview{max-width:100%;border-radius:10px;border:1px solid #333;display:none}.result{font-family:monospace;white-space:pre-wrap;background:#101010;border-radius:8px;padding:10px;min-height:52px}
table{width:100%;border-collapse:collapse;margin-top:8px}td,th{border-bottom:1px solid #2a2a2a;padding:8px;text-align:left;font-size:.92rem}th{color:#aaa;font-weight:500}.right{text-align:right}
.small{font-size:.82rem;color:#999} .pill{display:inline-block;background:#222;border:1px solid #333;padding:3px 7px;border-radius:999px;margin:2px;color:#bbb}
</style>
</head>
<body>
<div class="container">
  <div class="card">
    ...
    <div class="muted">Khong dung camera. Anh upload se duoc detect bang YuNet, align, augment, roi trich embedding bang MobileFaceNet.</div>
    <div style="margin-top:10px"><a href="/logout" style="color:#93c5fd;text-decoration:none">Dang xuat admin</a></div>
  </div>
  ...
  <div class="row">
    <div class="card">
      ...
      <form id="form">
        ...
        <input id="name" name="name" type="text" placeholder="VD: tra" required maxlength="50">
        ...
        <label><input id="append" name="append" type="checkbox"> Them vao nguoi cu neu da ton tai</label>
        <div style="margin-top:12px"><button id="btn" type="submit">Upload va dang ky</button></div>
      </form>
    ...
    <div class="card">
      ...
      <div style="margin-top:8px">
        ...
        <span class="pill">Image augmentation</span>
        <span class="pill">MobileFaceNet embedding</span>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <table>
      <thead><tr><th>Ten</th><th>So embedding</th><th class="right">Thao tac</th></tr></thead>
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    const r = await fetch('/upload_register', {method:'POST', body:fd});
...
async function loadDB(){
  ...
  Object.entries(db).forEach(([name,count]) => {
    ...
    tr.innerHTML = `<td>${name}</td><td>${count}</td><td class="right"><button class="danger" onclick="delPerson('${name.replaceAll("'", "\\'")}')">Xoa</button></td>`;
    body.appendChild(tr);
  });
  if (Object.keys(db).length === 0) body.innerHTML = '<tr><td colspan="3" class="muted">Database rong</td></tr>';
}
async function delPerson(name){
  if(!confirm('Xoa ' + name + '?')) return;
  const r = await fetch('/delete', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name})});
  const d = await r.json();
...
class YuNetDetector:
    def __init__(self, score_threshold: float = 0.55):
        ...
        self.det = cv2.FaceDetectorYN_create(
            str(YUNET_PATH), "", (320, 320), score_threshold, 0.30, 5000
        )
    ...
    def detect(self, frame: np.ndarray) -> List[Dict]:
        ...
        for f in faces:
            ...
            if not np.all(np.isfinite(landmarks)):
                landmarks = None
            results.append({"box": (x, y, bw, bh), "landmarks": landmarks, "score": score})
        return results
...
def augment_face(face_bgr: np.ndarray, max_count: int = 16) -> List[np.ndarray]:
    ...
    return variants[:max(1, min(max_count, len(variants)))]


def try_find_face(detector: YuNetDetector, img_bgr: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[Dict], np.ndarray]:
    # Thu detect anh goc va anh xoay neu upload co EXIF/orientation la.
    candidates = [img_bgr]
    candidates += [cv2.rotate(img_bgr, cv2.ROTATE_90_CLOCKWISE),
...
def load_db() -> Dict[str, List]:
    ...
    with open(DB_PATH, "r", encoding="utf-8") as f:
        raw = json.load(f)
    # Chuan hoa ve format {name: [embedding, ...]}
    db = {}
...
def login_required(fn):
    ...
    def wrapper(*args, **kwargs):
        if not session.get("admin_logged_in"):
            # API routes tra JSON, trang web thi redirect ve login.
...
def upload_register():
    ...
    if crop is None or face is None:
        return jsonify({"ok": False, "msg": "Khong tim thay khuon mat trong anh. Hay dung anh ro hon, mat lon hon, it nghieng hon."})
    ...
    print(f"[UPLOAD] name={name}, aug={len(embeddings)}, append={append}, score={face['score']:.3f}, ratio={ratio:.3f}")
    return jsonify({
        "ok": True,
        "name": name,
...
def get_db():
    db = load_db()
    return jsonify({name: len(embs) for name, embs in db.items()})
...
def main():
    ...
    p.add_argument("--score-threshold", type=float, default=0.55, help="Nguong YuNet cho anh upload. Giam neu anh kho detect.")
    args = p.parse_args()
    ...
    if ADMIN_PASSWORD == "admin123":
        print("[WARN] Dang dung mat khau mac dinh admin123. Hay doi bang: export ADMIN_PASSWORD=mat_khau_manh")
