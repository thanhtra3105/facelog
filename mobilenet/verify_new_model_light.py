...
Ban verify cho MODEL MOI: output_model.tflite
Giao dien/chuc nang giong file verify_light.py cua model cu:
- Flask render templates/ui.html
- Stream MJPEG /video_feed
- /status JSON cho giao dien
- VL53L0X pause/resume
- GPIO relay/LED
- YuNet detect + align 112x112
- MobileFaceNet model moi output 128D
- Ghi log ket qua de danh gia model

Chay test:
    python3 verify_new_model.py --no-gpio --no-sensor

Chay voi cam bien, khong relay:
    python3 verify_new_model.py --no-gpio
...
# Nen tao database rieng cho model moi de khong bi tron voi model cu
...
RESULT_LOG_PATH = BASE_DIR / "result_new_model.csv"
...
def append_result_log(row: dict):
    """
    Ghi log de sau nay so sanh 2 model.
    """
...
# =========================
class VL53L0XSensor:
    def __init__(self, min_distance_mm: int = 100, max_distance_mm: int = 1500):
...
class MobileFaceNet:
    def __init__(self, model_path: Path, num_threads: int = 2):
        ...
        print("[INFO] MobileFaceNet NEW ready")
...
def load_database(db_path: Path, expected_dim=None):
    if not db_path.exists():
        raise FileNotFoundError(
            f"Khong tim thay database: {db_path}\n"
            "Hay tao database rieng cho model moi, vi output_model.tflite la embedding 128D."
        )
...
def inference_loop(args):
    ...
    print("[INFO] Loading MobileFaceNet NEW...")
    ...
    print("[INFO] Verify NEW model started")
    print(
        f"[INFO] model={MODEL_PATH.name}, "
        f"db={Path(args.db).name}, "
    ...
    while True:
        ...
        # # Chuyen sang BGR cho OpenCV / YuNet / model.
        # # Neu dung USB camera OpenCV thi frame co the da la BGR;
        # # voi PiCamera RGB888 thi day la RGB.
        # if args.input_rgb:
            ...
            if face is not None:
                ...
                if crop is not None:
                    ...
                    if name:
                        ...
                        msg = f"[OK][NEW] {time.strftime('%H:%M:%S')} {name} sim={sim:.3f}"
                        print(msg)
                        new_log = msg
                        ...
                        msg = f"[DENY][NEW] {time.strftime('%H:%M:%S')} sim={sim:.3f}"
                        print(msg)
                        new_log = msg
...
def main():
    ...
    p.add_argument("--detect-max-distance", type=int, default=1500)
    ...
    # PiCamera RGB888 thi de True.
    # Neu dung USB camera va mau bi sai, chay them --input-bgr.
    p.add_argument("--input-rgb", dest="input_rgb", action="store_true", default=True)
    p.add_argument("--input-bgr", dest="input_rgb", action="store_false")
    ...
    print("[INFO] Model moi:", MODEL_PATH)
    print("[INFO] Database moi:", args.db)
    print("[INFO] Log ket qua:", RESULT_LOG_PATH)
