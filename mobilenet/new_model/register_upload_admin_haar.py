#!/usr/bin/env python3
"""
register_upload_admin_haar.py

Dang ky khuon mat bang anh upload, co dang nhap admin.
Ban nay dung model moi: output_model.tflite
Va dung Haar Cascade crop de khop voi stream_light.py dang verify bang Haar.
...
Chay:
    ...
    python3 register_upload_admin_haar.py
...
Can co cung thu muc:
    output_model.tflite
    face_database.json  # se duoc tao neu chua co
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Dang ky khuon mat Haar</title>
<style>
*{box-sizing:border-box}
body{
    margin:0;
...
<div class="container">
  <div class="card">
    <h1>Dang ky khuon mat bang Haar crop</h1>
    <div class="muted">
      Ban nay dung model <b>output_model.tflite</b>, embedding 128 chieu.
      Anh upload se duoc detect bang <b>Haar Cascade</b>, crop mat, augment,
      roi trich embedding bang MobileFaceNet. Pipeline nay khop voi stream_light.py dang verify bang Haar.
    </div>
  ...
  <div class="row">
    <div class="card">
      ...
      <div style="margin-top:12px" class="small">
        Nen dung anh nhin thang, ro mat, anh sang tot. Haar yeu hon YuNet nen anh nghieng/che mat co the kho detect.
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div style="margin-top:8px">
        <span class="pill">Haar detect</span>
        <span class="pill">Haar crop 112x112</span>
  ...
  try {
    ...
    if (d.ok) {
      ...
      result.textContent =
        ...
        `Face score: ${Number(d.face_score).toFixed(3)}\n` +
        `Ratio: ${Number(d.face_ratio).toFixed(3)}`;
      loadDB();
    } else {
...
class HaarFaceDetector:
    def __init__(self):
        ...
        if cascade_path is None:
            raise RuntimeError(
                ...
                "Hoac kiem tra bang:\n"
                "sudo find /usr -name 'haarcascade_frontalface_default.xml'"
            )
            ...
            results.append(
                {
                    ...
                    # Haar khong co score thuc, gan 1.0 de khong loi giao dien
                    "score": 1.0,
                }
            )
    ...
    @staticmethod
    def crop(frame_bgr: np.ndarray, face: Dict, pad_ratio: float = 0.25) -> Optional[np.ndarray]:
        x, y, w, h = face["box"]
...
def augment_face(face_bgr: np.ndarray, max_count: int = 16) -> List[np.ndarray]:
    """
    Augment tren crop Haar 112x112.
    Nen dung augmentation vua phai, vi Haar crop khong align landmark.
    """
    ...
    variants = [
        ...
        adjust_bc(face, 1.0, -18),
        adjust_bc(face, 1.10, 0),
        adjust_bc(face, 0.90, 0),
        rotate_bound(face, -5),
        rotate_bound(face, 5),
        rotate_bound(face, -3),
        rotate_bound(face, 3),
        shift_image(face, -4, 0),
        shift_image(face, 4, 0),
        shift_image(face, 0, -4),
        shift_image(face, 0, 4),
        cv2.GaussianBlur(face, (3, 3), 0),
        add_noise(face, 2.5),
        cv2.flip(face, 1),
        adjust_bc(rotate_bound(face, -4), 1.05, 8),
        adjust_bc(rotate_bound(face, 4), 0.95, -6),
        add_noise(adjust_bc(face, 1.05, 8), 2.0),
...
def try_find_face(detector: HaarFaceDetector, img_bgr: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[Dict], np.ndarray]:
    """
    Thu anh goc va 3 goc xoay, vi anh upload co the sai orientation.
    Dung Haar crop de khop voi stream_light.py.
    """
    candidates = [
        img_bgr,
    ...
    for cand in candidates:
        ...
        crop = detector.crop(cand, face, pad_ratio=0.25)
    ...
    for name, embs in db.items():
        ...
        for e in embs:
            try:
                ...
                if arr.ndim == 1 and arr.shape[0] == EXPECTED_EMB_DIM:
                    arr = arr / (np.linalg.norm(arr) + 1e-8)
                    valid.append(arr.tolist())
                else:
                    print(f"[WARN] Bo qua embedding cu cua {name}, shape={arr.shape}")
...
_detector: Optional[HaarFaceDetector] = None
...
def upload_register():
    ...
    if crop is None or face is None:
        return jsonify(
            {
                "ok": False,
                "msg": "Khong tim thay khuon mat bang Haar. Hay dung anh nhin thang, ro mat, anh sang tot hon.",
            }
        )
    ...
    print(
        f"[UPLOAD][HAAR] name={name}, aug={len(embeddings)}, append={append}, "
...
def main():
    ...
    # Giu tham so nay de tuong thich lenh cu, nhung ban Haar khong dung.
    p.add_argument(
        "--score-threshold",
        type=float,
        default=0.55,
        help="Khong dung trong ban Haar. Giu de tuong thich lenh cu.",
    )
    ...
    print("[INFO] Detector: Haar Cascade")
